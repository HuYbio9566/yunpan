...
function layout(content) {
  return `
    <div class="app-shell contract-shell">
      ...
      <main class="main"><header class="topbar"><div class="breadcrumb"><a href="index.html">AI 工具中心</a><span>/</span><strong>合同风险审阅</strong></div><nav class="primary-nav"><a class="primary-nav-link" href="report.html">调研报告</a><a class="primary-nav-link active" href="index.html">新增工具</a><a class="primary-nav-link" href="existing-tools.html">已有工具</a></nav><div class="top-actions"><button class="btn" data-action="toast" data-toast="已打开审阅记录">使用记录</button><button class="btn" data-action="toast" data-toast="审阅边界与帮助已打开">使用帮助</button></div></header>${content}</main>
    </div>
...
function renderEntry() {
  return `
    <section class="contract-entry">
      <div class="contract-entry-head"><p class="contract-eyebrow">条款与红线审查台</p><h1>依据企业 Playbook 审阅合同，直接形成可谈判的修改稿</h1><p>AI 定位风险、说明规则依据并生成红线建议；法务和业务人员保留最终决策。</p></div>
      <div class="contract-start-grid">
        <main class="contract-upload-card">
          <div class="selected-contract"><span class="word-file"><i data-lucide="file-text"></i></span><div><strong>星云数据平台服务采购协议_v3.docx</strong><p>15 页 · 386 KB · 来自企业云盘 / 采购部 / 待审合同</p></div><button class="icon-button" data-action="toast" data-toast="已打开云盘文件选择器"><i data-lucide="refresh-cw"></i></button></div>
        ...
        <aside class="review-config">
          ...
          <label><span>合同类型</span><select data-config="type"><option>SaaS 服务采购</option><option>软件许可</option><option>数据处理协议</option><option>教育内容合作</option></select></label>
