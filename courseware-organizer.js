...
function moveResource(resource, lessonId, kind) {
  return { ...resource, lessonId, kind, status: "ready", tags: ["AI 已分类"] };
}
...
function handleClick(event) {
  ...
  if (action === "start-organize") state.screen = "workspace";
    ...
    state.modal = null;
    if (!state.resources.some(item => item.lessonId === "lesson-1" && item.kind === "answer")) state.resources.push({ id: "generated-answer", lessonId: "lesson-1", kind: "answer", name: "二次函数图像练习_答案解析.docx", size: "520 KB", updated: "刚刚", pages: 7, status: "ready", tags: ["AI 生成", "待教师复核"] });
