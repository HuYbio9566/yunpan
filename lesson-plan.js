...
function layout(content) {
  return `<div class="app-shell lesson-shell">${renderProductSidebar()}<main class="main"><header class="topbar"><div class="breadcrumb"><a href="index.html">AI 工具中心</a><span>/</span><strong>教案生成工具</strong></div><nav class="primary-nav"><a class="primary-nav-link" href="report.html">调研报告</a><a class="primary-nav-link active" href="index.html">新增工具</a><a class="primary-nav-link" href="existing-tools.html">已有工具</a></nav><div class="top-actions"><button class="btn" data-action="toast" data-toast="已打开教案记录">历史教案</button><button class="btn" data-action="toast" data-toast="教学设计帮助已打开">使用帮助</button></div></header>${content}</main></div>${state.modal ? renderModal() : ""}${state.toast ? `<div class="toast">${state.toast}</div>` : ""}`;
}

function renderSetup() {
  return `<section class="lesson-setup"><div class="setup-intro"><p class="lesson-eyebrow">结构化教学设计</p><h1>从可信课程材料出发，生成可调整、可核验的课堂教案</h1><p>先补齐课题、材料与班情，AI 再生成目标、时间轴、评价和分层作业。</p></div><div class="setup-stepper">${["课题与课时", "课程材料与课标", "班情与教学偏好"].map((label, index) => `<button class="${state.setupStep === index + 1 ? "active" : state.setupStep > index + 1 ? "done" : ""}" data-action="setup-step" data-step="${index + 1}"><span>${state.setupStep > index + 1 ? `<i data-lucide="check"></i>` : index + 1}</span><strong>${label}</strong></button>`).join("")}</div><div class="setup-card">${state.setupStep === 1 ? renderSetupBasics() : state.setupStep === 2 ? renderSetupMaterials() : renderSetupClass()}</div></section>`;
}

function renderSetupBasics() {
  return `<div class="setup-panel"><div class="setup-panel-head"><span class="setup-icon"><i data-lucide="book-marked"></i></span><div><h2>这节课要教什么？</h2><p>结构化信息会直接决定目标、活动和课时节奏。</p></div></div><div class="setup-form-grid"><label class="wide"><span>课题</span><input value="函数的概念与定义域"></label><label><span>学科</span><select><option>高中数学</option><option>初中数学</option><option>高中物理</option></select></label><label><span>年级</span><select><option>高一上学期</option><option>高二上学期</option></select></label><label><span>课时长度</span><select><option>45 分钟</option><option>40 分钟</option><option>90 分钟</option></select></label><label><span>课型</span><select><option>新授课</option><option>复习课</option><option>探究课</option></select></label></div><div class="setup-summary"><i data-lucide="sparkles"></i><div><strong>AI 已识别建议教学主题</strong><p>函数定义、对应关系、定义域限制与真实情境中的变量关系。</p></div></div><div class="setup-actions"><span></span><button class="btn primary" data-action="next-step">下一步：选择课程材料<i data-lucide="arrow-right"></i></button></div></div>`;
}
...
function renderSectionContent(section) {
  ...
  return `<section class="paper-section"><div class="paper-section-head"><div><span>${String(state.sections.findIndex(item => item.id === section.id) + 1).padStart(2, "0")}</span><h3>${section.label}</h3>${section.status === "ai" ? `<em class="ai-draft">AI 草稿 · 待教师复核</em>` : ""}</div></div><div class="content-blocks">${Array.from({ length: content.length / 2 }, (_, index) => `<section><h4>${content[index * 2]}</h4><p contenteditable="true">${content[index * 2 + 1]}</p><span class="source-chip"><i data-lucide="link-2"></i>${index ? "课程标准节选.pdf" : "函数基础_第1课时.pptx"}</span></section>`).join("")}</div></section>`;
}

function renderAssistant(validation) {
  const section = state.sections.find(item => item.id === state.activeSection) || state.sections[0];
  return `<aside class="lesson-assistant lesson-pane-assistant"><div class="assistant-head"><div><span class="assistant-mark"><i data-lucide="sparkles"></i></span><div><h2>AI 教学助理</h2><p>正在协助：${section.label}</p></div></div><button class="icon-button" data-action="toast" data-toast="AI 助手设置已打开"><i data-lucide="settings-2"></i></button></div>${state.activeSection === "timeline" ? renderTimelineSuggestions(validation) : renderGeneralSuggestions(section)}<section class="assistant-section source-check"><div class="assistant-title"><h3>生成依据</h3><button data-action="show-sources">4 个来源</button></div><div class="source-coverage"><div><span>教材内容</span><strong>92%</strong></div><i><b style="width:92%"></b></i></div><div class="source-coverage"><div><span>课程标准</span><strong>100%</strong></div><i><b style="width:100%"></b></i></div><button class="assistant-action" data-action="show-sources"><i data-lucide="scan-search"></i><span><strong>检查课标与材料引用</strong><small>查看每段内容的生成依据</small></span><i data-lucide="chevron-right"></i></button></section><section class="assistant-section teacher-boundary"><i data-lucide="shield-check"></i><div><strong>教师确认边界</strong><p>AI 内容需由教师结合班情复核，尤其是目标、时间和评价标准。</p></div></section><div class="assistant-input"><textarea placeholder="告诉 AI 你想如何调整当前模块…"></textarea><button data-action="toast" data-toast="AI 已收到修改要求"><i data-lucide="arrow-up"></i></button></div></aside>`;
}
